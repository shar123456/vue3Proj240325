<template>
  ...
  align-items:center;width: 100%;" ref='listRef' v-model:loading="loading" :finished="finished" 
...
border-radius: 11px;padding: 3px;">
        ...
        <van-row>
          ...
          <van-col class="colDesc" span="8"><van-text-ellipsis :content="Temp.name" /></van-col>  
          <van-col class="colTitle" span="4">职务：</van-col>
          <van-col class="colDesc" span="8"><van-text-ellipsis :content="Temp.position" /></van-col>  
        </van-row>
        <van-row>
          <van-col class="colTitle" span="4">行业：</van-col>
          <van-col class="colDesc" span="8"><van-text-ellipsis :content="Temp.industry" /></van-col>  
        ...
        <van-row>
          <van-col class="colTitle colTitleEnd" span="6">审核状态：</van-col>
          <van-col class="colDesc colTitleEnd" span="6"
          :class="{ cGreen: Temp.clueAuditState == '通过', cRed: Temp.clueAuditState == '未审核', cBlue: Temp.clueAuditState == '驳回' }"
          ><van-text-ellipsis :content="Temp.clueAuditState" /></van-col>  
          <van-col style="display: flex;
justify-content:flex-end;padding-right: 4px;align-items: center;" class="colTitle colTitleEnd" span="12"><van-button @click="DetailBtn(Temp.clueCode,Temp.id)" type="success" size="mini" >线索详情</van-button></van-col>
        </van-row>
      </div>
...
export default {
  ...
  setup() {
    ...
    const onLoad = async () => {
      ...
      DataEntityState.QueryConditionInfo.firm=searchData.value;

        if(DataEntityState.QueryConditionInfo.firm!=null&&DataEntityState.QueryConditionInfo.firm!="")
{
  DataListTemp = UserDatasList.filter((i:any)=>i.firm.indexOf(DataEntityState.QueryConditionInfo.firm)>0);
}
else
{
...
console.log("DataListTemp.value",DataListTemp.datas);
    ...
    watch(refreshMark, () => {
      ...
      GetClueManagementDatas({
        ...
        if (res.isSuccess) {
          ...
          console.log("GetClueManagementDatas", DataEntityState.DataList);
          if (totalCount.value > 0) {
            ShowEmpty.value = false;
    ...
    let SearchBtn = () => {
      ...
      DataEntityState.QueryConditionInfo.firm=txt;
      ...
      GetClueManagementDatas({
        ...
        if(DataEntityState.QueryConditionInfo.firm!=null&&DataEntityState.QueryConditionInfo.firm!="")
{
  DataListTemp= res.datas.filter((i:any)=>i.firm.indexOf(DataEntityState.QueryConditionInfo.firm)>0);
}
else
{
