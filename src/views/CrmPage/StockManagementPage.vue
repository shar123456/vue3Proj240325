...
configGridModal,configExportModal,
  ...
  const router=useRouter();
    const DataEntityState = reactive(new StockEntity());
 
    let  NewDataEntityState=new StockEntity();
...
// }
let columnList=deepClone(StockColumns)
      ...
      //获用户数据
      loading.value = true;
      let UserDatasList = await GetStockManagementDatas({
        current: 1,
        pageSize: pageSize.value,
        ...DataEntityState.QueryConditionInfo,
      });
...
const SearchBtn = async (payload: any) => {
    DataEntityState.selectedRowKeys = [];
      ...
      let UserDatasList1 = await GetStockManagementDatas({
        current: 1,
        pageSize: pageSize.value,
        ...payload,
      });
