 ...
 @BatchDelete="BatchDeleteBtn"
  ...
  <div id="QuotesManaDataList">
    ...
    >
      ...
      <template v-if="column.dataIndex === 'quoteSource'">
        <span>
          <a-tag
           
          :color="(record.quoteSource=='手动'||record.quoteSource=='导入') ? 'geekblue' :'green' ">
          
            {{ record.quoteSource}}
          </a-tag>
        </span>
      </template>
      <template v-if="column.dataIndex === 'quoteState'">
        <span>
          <a-tag
           
          :color="record.quoteState=='启用' ? 'geekblue' :'volcano' ">
      ...
      <template v-if="column.dataIndex === 'productCode'">
        <span>
          ...
          :color="record.productCode=='' ? 'geekblue' :'green' ">
      ...
      <template v-if="column.dataIndex === 'customerCode'">
        <span>
          ...
          :color="record.customerCode=='' ? 'geekblue' :'green' ">
...
<a  @click="DeleteBth(record.id,record.quoteCode)"
...
<configGridModal
 ...
 <ExportExcelModal
    ...
    urlData="/QuotesManagement/UpLoadFile"
      configType="QuotesManagement"
  />
...
import {
  QuotesEntity,QuotesColumns,ExportColumns
} from "../../TypeInterface/ICrm/IQuotesManagement";
...
import {
  GetQuotesManagementDatas,AddQuotes,UpdateQuotes,DeleteById,BatchDelete,BatchExport,CopyDataById,
...
export default defineComponent({
  components: {
configGridModal,configExportModal,ExportExcelModal,
  ...
  const router=useRouter();
    const DataEntityState = reactive(new QuotesEntity());
 
    let  NewDataEntityState=new QuotesEntity();
...
onMounted(async () => {
      //获取表格列及处理表格列
      let columnList = await GetLoginRecordColumn({ pageName: "QuotesManagement" });
      console.log("ProductManagementColumn1",columnList)
if(columnList==undefined||columnList.length==0)
{
  columnList=deepClone(QuotesColumns)
}
...
let ExportColumnsList = await GetExpColumnsConfig({
     ...
     // console.log("ExportColumnsList", ExportColumnsList);
      ...
      //获用户数据
      loading.value = true;
      let UserDatasList = await GetQuotesManagementDatas({
        current: 1,
        pageSize: pageSize.value,
        ...DataEntityState.QueryConditionInfo,
      });
...
const SearchBtn = async (payload: any) => {
 ...
 DataEntityState.selectedRowKeys = [];
      ...
      let UserDatasList1 = await GetQuotesManagementDatas({
        current: 1,
        pageSize: pageSize.value,
        ...payload,
      });
 ...
 const CreateBtn = (payload: any) => {
      router.push({ path: "/Home/CreateQuotesPage", query: {pageType:"add"} });
    };
...
router.push({ path: "/Home/CreateQuotesPage", query: {pageType:"edit",id: item} });
  ...
  const ExportExcelBtn = () => {
      ...
      Modal.confirm({
        ...
        onOk() {
         ...
         const fileName ="报价单-"+new Date().getTime();
...
#QuotesManaDataList .ant-table-thead > tr > th,
#QuotesManaDataList .ant-table-tbody > tr > td {
  padding: 9px 9px;
}
.table-striped td {
}
